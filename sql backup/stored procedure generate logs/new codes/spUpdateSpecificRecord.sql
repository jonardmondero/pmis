...
BEGIN
 /*   IF(state = 'inAM')THEN
	...
	*/
	CASE state
	WHEN 'inAM' THEN
	UPDATE dailytimerecord SET inAM = dtrtime WHERE DATE = ddate AND employeeNo = empno LIMIT 1;
	WHEN 'outAM' THEN
	UPDATE dailytimerecord SET outAM = dtrtime WHERE DATE = ddate AND employeeNo = empno LIMIT 1;
	WHEN 'inPM' THEN
	UPDATE dailytimerecord SET outPM = dtrtime WHERE DATE = ddate AND employeeNo = empno LIMIT 1;
	WHEN 'outPM' THEN
	UPDATE dailytimerecord SET outPM = dtrtime WHERE DATE = ddate AND employeeNo = empno LIMIT 1;
	WHEN 'otIn' THEN
	UPDATE dailytimerecord SET otIn = dtrtime WHERE DATE = ddate AND employeeNo = empno LIMIT 1;
	WHEN 'otOut' THEN
	UPDATE dailytimerecord SET otOut = dtrtime WHERE DATE = ddate AND employeeNo = empno LIMIT 1;	
