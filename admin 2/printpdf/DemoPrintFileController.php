...
if (isset($urlParts['query'])) {
    ...
    if (isset($qs[WebClientPrint::CLIENT_PRINT_JOB])) {
        ...
        if ($qs['filetype'] === 'PDF') {
            if($qs['period'] === '1'){
           $file_url =  "http://localhost/pmis/plugins/jasperreport/dtrreport.php?"."empno=".$qs['empno']."&year=".$qs['year']."&days=".$qs['days'];
        }else if($qs['period']=== '2'){
            $file_url =  "http://localhost/pmis/plugins/jasperreport/dtrreport1st.php?"."empno=".$qs['empno']."&year=".$qs['year'];
        }  else if($qs['period']=== '3'){
            $file_url =  "http://localhost/pmis/plugins/jasperreport/dtrreport2nd.php?"."empno=".$qs['empno']."&year=".$qs['year'];
        }
     
            // $url = 'http://localhost/PMIS/plugins/jasperreport/dtrreport.php?empno=10204480&year=2023-06-%&days=30';
           $url = $file_url;
        ...
        if (!Utils::isNullOrEmptyString($filePath)) {
            ...
            if ($qs['filetype'] === 'PDF')
            {
                ...
                $myfile->printRotation = PrintRotation::Rot270;
